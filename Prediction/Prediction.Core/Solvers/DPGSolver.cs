...
namespace Prediction.Core.Solvers
{
    ...
    /// </summary>
    public class DPGSolver : BaseSolver
    {
        public DPGSolver(IDixonManager dixonManager)
            : base(dixonManager)
        { }
        ...
        public override double Solve(DateTime actualDate)
        {
            ...
            Decision g = new Decision(Domain.RealRange(0, 2), "g", teams);
            g.SetBinding(_DixonManager.Teams, "Gamma", "Id");
            ...
            model.AddDecisions(attack, defence, g, gamma, mi);
            ...
            model.AddConstraint("gCount",
                Model.Sum(Model.ForEach(teams, t => g[t])) == _DixonManager.Teams.Count);
            ...
            Goal sum = model.AddGoal("sum", GoalKind.Maximize,
                Model.Sum(
                   ...
                   {
                       // home-attack
                       var lambda = mi * attack[h] * defence[a] * g[h] * gamma;
