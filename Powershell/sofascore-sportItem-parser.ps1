...
Function Download-Json($output)
{
    Write-Host "--- Stahuji JSON ---"
    $url = [string]::Format("http://www.sofascore.com/{0}/{1}/{2}/json", $sport, $category, $date)    
    $start_time = Get-Date
...
Function Init()
{
    Write-Host "--- Inicializuji ---"
    New-Item "Cache" -type Directory -Force | Out-Null
    $output = [string]$PSScriptRoot + [string]::Format("\Cache\{2}-{0}-{1}.json", $sport, $category, $date)
...
Function Process-Data($path)
{
    ...
    foreach ($i in $jsonObj.sportItem.tournaments) 
    {
